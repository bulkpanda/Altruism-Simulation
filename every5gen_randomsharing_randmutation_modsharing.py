...
Reproduction rate = 1 (mutation is random) with prob 1/4
    ...
    def reproduce(self,animals):
        ...
        if ran < 0.25:
            ...
            if anew>0 and anew <1:
                daughter = Animal(xnew,ynew,self.food/2,anew)
                animals.append(daughter)
            else:
                daughter = Animal(xnew,ynew,self.food/2,self.a)
                animals.append(daughter)
        else:
                daughter = Animal(xnew,ynew,self.food/2,self.a)
                animals.append(daughter)           
...
for j in range(iterations):
    ...
    """Share event"""
    """Three sharing events per iteration"""
    for  k in range(3):
    ...
    for i in range(len(animals)):
        animals[i].numberanimals(animals)
    """Plots graph every hundredth iteration"""
    if j%100 == 0:
        plt.figure(1)
        plt.clf()
        for i in range(len(animals)):
